...
const parser_byte_length_1 = __importDefault(
...
class SimpleSerialProtocol {
	...
	init(initilizationDelay = 2500) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			this.oneByteParser = this.serialPort.pipe(
				new serialport_1.ByteLengthParser({ length: 1 })
			);
	...
	dispose() {
		return __awaiter(this, void 0, void 0, function* () {
			...
			this.paramTypeInstances.clear();
			this.paramTypeInstances = null;

			if (this.isOpen()) {
				yield this.serialPort.close((err) =>
	...
	registerCommand(commandIdOrConfig, callback = null, paramTypes = null) {
		let commandId;
		if (
			commandIdOrConfig instanceof ReadCommandConfig_1.ReadCommandConfig
		) {
		...
		if (!callback) {
			throw new SimpleSerialProtocolError_1.SimpleSerialProtocolError(
				SimpleSerialProtocolError_1.SimpleSerialProtocolError.ERROR_CALLBACK_IS_NULL
			);
		}
		if (this.registeredCommands.has(commandId)) {
			throw new SimpleSerialProtocolError_1.SimpleSerialProtocolError(
				SimpleSerialProtocolError_1.SimpleSerialProtocolError.ERROR_COMMAND_IS_ALREADY_REGISTERED
			);
		}
		const registeredCommand = new RegisteredCommand_1.RegisteredCommand(
			commandId,
			callback,
			paramTypes
		);
	...
	writeCommand(commandIdOrConfig, params = null) {
		let commandId = '';
		if (
			commandIdOrConfig instanceof WriteCommandConfig_1.WriteCommandConfig
		) {
			commandId = commandIdOrConfig.getCommandId();
		...
		if (commandId.length !== 1) {
			throw new SimpleSerialProtocolError_1.SimpleSerialProtocolError(
				SimpleSerialProtocolError_1.SimpleSerialProtocolError.ERROR_WRONG_COMMAND_NAME_LENGTH
			);
		}
		this.write(
			this.paramTypeInstances
				.get(ParamTypeChar_1.ParamTypeChar.NAME)
				.getBuffer(commandId)
		);
		if (params) {
			for (const param of params) {
				if (ParamsParser_1.ParamsParser.hasType(param.type)) {
					const typeClassInstance = this.paramTypeInstances.get(
						param.type
					);
					...
					throw new SimpleSerialProtocolError_1.SimpleSerialProtocolError(
						SimpleSerialProtocolError_1.SimpleSerialProtocolError.ERROR_PARAM_TYPE_UNKNOWN
					);
				}
			}
		}
		this.write(
			this.paramTypeInstances
				.get(ParamTypeInt8_1.ParamTypeInt8.NAME)
				.getBuffer(SimpleSerialProtocol.CHAR_EOT)
		);
	}
	addParamType(name, clazz) {
		if (ParamsParser_1.ParamsParser.hasType(name)) {
			throw new SimpleSerialProtocolError_1.SimpleSerialProtocolError(
				SimpleSerialProtocolError_1.SimpleSerialProtocolError.ERROR_PARAM_TYPE_IS_ALREADY_REGISTERED
			);
		}
	...
	initParamTypes() {
		ParamsParser_1.ParamsParser.TYPES = new Map();

		this.addParamType(
			ParamTypeByte_1.ParamTypeByte.NAME,
			ParamTypeByte_1.ParamTypeByte
		);
		this.addParamType(
			ParamTypeBoolean_1.ParamTypeBoolean.NAME,
			ParamTypeBoolean_1.ParamTypeBoolean
		);
		this.addParamType(
			ParamTypeChar_1.ParamTypeChar.NAME,
			ParamTypeChar_1.ParamTypeChar
		);
		this.addParamType(
			ParamTypeString_1.ParamTypeString.NAME,
			ParamTypeString_1.ParamTypeString
		);
		this.addParamType(
			ParamTypeFloat_1.ParamTypeFloat.NAME,
			ParamTypeFloat_1.ParamTypeFloat
		);
		this.addParamType(
			ParamTypeInt8_1.ParamTypeInt8.NAME,
			ParamTypeInt8_1.ParamTypeInt8
		);
		this.addParamType(
			ParamTypeInt16_1.ParamTypeInt16.NAME,
			ParamTypeInt16_1.ParamTypeInt16
		);
		this.addParamType(
			ParamTypeInt32_1.ParamTypeInt32.NAME,
			ParamTypeInt32_1.ParamTypeInt32
		);
		this.addParamType(
			ParamTypeInt64_1.ParamTypeInt64.NAME,
			ParamTypeInt64_1.ParamTypeInt64
		);
		this.addParamType(
			ParamTypeUnsignedInt8_1.ParamTypeUnsignedInt8.NAME,
			ParamTypeUnsignedInt8_1.ParamTypeUnsignedInt8
		);
		this.addParamType(
			ParamTypeUnsignedInt16_1.ParamTypeUnsignedInt16.NAME,
			ParamTypeUnsignedInt16_1.ParamTypeUnsignedInt16
		);
		this.addParamType(
			ParamTypeUnsignedInt32_1.ParamTypeUnsignedInt32.NAME,
			ParamTypeUnsignedInt32_1.ParamTypeUnsignedInt32
		);
		this.addParamType(
			ParamTypeUnsignedInt64_1.ParamTypeUnsignedInt64.NAME,
			ParamTypeUnsignedInt64_1.ParamTypeUnsignedInt64
		);
	}
	write(buffer) {
		...
		if (this.currentCommand) {
			...
			if (this.currentCommand.paramsRead()) {
				...
				if (byte === SimpleSerialProtocol.CHAR_EOT) {
					...
					throw new SimpleSerialProtocolError_1.SimpleSerialProtocolError(
						SimpleSerialProtocolError_1.SimpleSerialProtocolError.ERROR_EOT_WAS_NOT_READ
					);
				}
			} else {
