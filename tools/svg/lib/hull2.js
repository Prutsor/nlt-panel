...
var concaveHull = (function () {
	...
	function calculate(pointsList, k) {
		...
		while (
			(!pointEquals(currentPoint, firstPoint) || step == 2) &&
			dataset.length > 0
		) {
			if (verbose) {
			...
			if (verbose) {
				let debug_string =
					'Nearest Points to ' + getIndex(pointsList, currentPoint) + ': ';
			...
			let debug_string =
				'Sorted Candidate Points from ' +
				getIndex(pointsList, currentPoint) +
				': ';
					...
					if (verbose) {
						...
						console.log(
							'Evaluating intersection of line ' +
								getIndex(pointsList, hull[step - 2]) +
								'->' +
								getIndex(pointsList, cPoints[i - 1]) +
								' and line ' +
								getIndex(pointsList, hull[step - 2 - j]) +
								'->' +
								getIndex(pointsList, hull[step - 1 - j])
						);
					}
			...
			console.log(
				'previousAngle: ',
				(previousAngle * (180 / Math.PI)).toFixed(2)
			);
		...
		while (allInside === true && i >= 0) {
			if (verbose) {
				console.log('PointInPolygonQ dataset: ', dataset);
			}
	...
	function SortByAngle(points, point, prev_angle, PointsListDebug) {
		...
		for (let p = 0; p < points.length; p++) {
			let obj = {
				...
				raw_angle: (Angle(points[p], point) * (180 / Math.PI)).toFixed(2),
