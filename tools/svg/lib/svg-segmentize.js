...
(function (global, factory) {
	...
	var svg_path_to_segments = function svg_path_to_segments(path) {
		var RESOLUTION =
			arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 128;
		var d = path.getAttribute('d');
		var prop = PathProperties(d);
		var length = prop.getTotalLength();
		var isClosed = d[d.length - 1] === 'Z' || d[d.length - 1] === 'z';
		var segmentLength = isClosed
			? length / RESOLUTION
			: length / (RESOLUTION - 1);
		var pathsPoints = Array.from(Array(RESOLUTION))
			.map(function (_, i) {
	...
	var segmentize = function segmentize(input) {
		...
		var RESOLUTION =
			_typeof(options.resolution) === 'object' ? options.resolution : {};
		...
		var lineSegments = elements
			.filter(function (el) {
				return parseable.indexOf(el.tagName) !== -1;
			})
			.map(function (el) {
				return parsers[el.tagName](el, RESOLUTION[el.tagName])
					.map(function (unit) {
